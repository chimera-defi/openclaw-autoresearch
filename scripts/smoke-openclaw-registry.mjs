...
const [versionInput, hostRootInput] = process.argv.slice(2);
...
if (!versionInput || !hostRootInput) {
  fail("Usage: npm run smoke:registry-openclaw-host -- <published-version> /path/to/openclaw");
}
...
if (versionInput !== packageVersion) {
  process.stderr.write(
    `Warning: testing ${packageName}@${versionInput}, package.json is currently ${packageVersion}\n`,
  );
}
...
const smokeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "openclaw-autoresearch-registry-smoke-"));
...
try {
  run("npm", ["pack", `${packageName}@${versionInput}`, "--pack-destination", smokeRoot]);
  const tarball = fs
  ...
  process.stdout.write(`OpenClaw registry smoke passed: ${packageName}@${versionInput}\n`);
  process.stdout.write(`Host: ${hostRoot}\n`);
